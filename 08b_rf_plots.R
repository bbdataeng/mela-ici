# load("nonsync/08b_plots_RF_metadaset.RData") # run to restore working space
...
load("nonsync/08a_RF_metadataset.RData") # load random forests data


for (x in seq_along(to_do_list)) {
  # add RF short name in to_do_list[[x]]
  to_do_list[[x]]$rf_short <- substr(
    to_do_list[[x]]$rf_formula,
    start = 1, stop = 3
  ) |> factor()
  # keep only binary RFs in to_do_list[[x]]
  to_do_list[[x]] <- subset(to_do_list[[x]], response_type == "binary")
  rownames(to_do_list[[x]]) <- to_do_list[[x]]$rf_short
}
...
rf_formulae_colors <- paletteer_d("RColorBrewer::Set2", nlevels(to_do_list[[1]]$rf_short))
names(rf_formulae_colors) <- levels(to_do_list[[1]]$rf_short)
...
for (x in seq_along(to_do_list)) {
  ...
  formulae_data <- cbind(
    LM22 = rep(TRUE, nrow(to_do_list[[x]])),
    to_do_list[[x]][, c("age_and_gender", "hed_data", "technical_predictors")]
  ) |> as.matrix()
  ...
  # draw heatmap
  png(file.path(outdirs[x], "RF_formulae_heatmap.png"),
...
for (x in seq_along(to_do_list)) {
  for (i in seq_len(nrow(to_do_list[[x]]))) {
    png(file.path(to_do_list[[x]]$folder[i], paste0("ROC_plot_", to_do_list[[x]]$rf_formula[i], ".png")),
      width = 3 * resol, height = 3 * resol, res = resol
    )
    plot(rocs_list[[x]][[i]],
      print.auc = TRUE,
      auc.polygon = TRUE,
      main = to_do_list[[x]]$rf_short[i],
      xaxs = "i", yaxs = "i", las = 1,
...
# all together
for (x in seq_along(to_do_list)) {
  png(file.path(outdirs[x], "ROC_plots.png"),
...
median_imp <- lapply(vimp_df_list, function(a) {
  apply(a, 2, median, na.rm = TRUE) |> sort()
})

# make barplot
for (x in seq_along(to_do_list)) {
  png(
    file.path(outdirs[x], "median_importance_barplot.png"),
    width = 6 * resol, height = 8 * resol, res = resol
  )
  par(mar = c(3, 15.5, 0.5, 0.5), las = 1, mgp = c(2, 1, 0))
  xx <- barplot(median_imp[[x]], horiz = TRUE, plot = FALSE)
  varnames <- ifelse(names(median_imp[[x]]) %in% names(cell_types_original),
    cell_types_original[names(median_imp[[x]])],
    names(median_imp[[x]])
  )
  plot(NULL,
    xlim = c(0, 100),
...
for (x in seq_along(to_do_list)) {
  png(
    file.path(outdirs[x], "median_importance_dotplot.png"),
    width = 6 * resol, height = 8 * resol, res = resol
  )
  ...
  plot(NULL,
    xlim = c(1, 100),
    ylim = c(0, length(median_imp[[x]])) + 0.5, yaxt = "n", yaxs = "i",
  ...
  segments(
    ...
    y0 = seq_along(median_imp[[x]]), y1 = seq_along(median_imp[[x]]),
    col = "lightgray", lty = "dotted"
  )
  segments(
    x0 = rep(0, length(median_imp[[x]])), x1 = median_imp[[x]],
    y0 = seq_along(median_imp[[x]]), y1 = seq_along(median_imp[[x]]),
    col = "#4C72B0", lwd = 5, lend = 0
  )
  set.seed(123456)
  for (i in seq_along(median_imp[[x]])) {
    points(
      x = vimp_df_list[[x]][, names(median_imp[[x]])[i]],
      y = jitter(rep(i, nrow(vimp_df_list[[x]])), amount = 0.15),
...
xxrank <- lapply(vimp_list, function(a) {
  lapply(
    X = a, FUN = function(b) {
      xrank <- rank(b, na.last = TRUE)
      xrank[is.na(b)] <- NA
...
for (x in seq_along(to_do_list)) {
  for (i in seq_len(nrow(to_do_list[[x]]))) {
    ...
    png(
      file.path(to_do_list[[x]]$folder[i], paste0(
        "vimp_barplot_", to_do_list[[x]]$rf_formula[i], ".png"
      )),
      width = 6 * resol, height = 8 * resol, res = resol
    )
    par(mar = c(3, 15.5, 1.5, 0.5), las = 1)
    xx <- barplot(vimp_df_list[[x]][i, ], horiz = TRUE, plot = FALSE)
    xlim <- range(vimp_df_list[[x]][i, ], na.rm = TRUE)
    xlim <- xlim + c(-diff(xlim) * 0.1, diff(xlim) * 0.1)
    varnames <- ifelse(names(vimp_df_list[[x]][i, ]) %in% names(cell_types_original),
      cell_types_original[names(vimp_df_list[[x]][i, ])],
      names(vimp_df_list[[x]][i, ])
    )
    plot(NULL,
    ...
    barplot(vimp_df_list[[x]][i, ],
      horiz = TRUE, add = TRUE, names.arg = varnames,
      col = rf_formulae_colors[i]
    )
    text(
      x = as.numeric(vimp_df_list[[x]][i, ]), y = xx, labels = xxrank[[x]][[i]],
      pos = ifelse(vimp_df_list[[x]][i, ] >= 0 | is.na(vimp_df_list[[x]][i, ]), 4, 2)
    )
    dev.off()
    # make barplot of top 10 importance scores
    png(
      file.path(to_do_list[[x]]$folder[i], paste0(
        "vimp_barplot_top10_", to_do_list[[x]]$rf_formula[i], ".png"
      )),
      width = 6 * resol, height = 4 * resol, res = resol
    )
    par(mar = c(3, 15.5, 1.5, 0.5), las = 1)
    top10 <- vimp_df_list[[x]][i, ] |>
...
metrics_binary <- lapply(seq_along(to_do_list), function(x) {
  metr <- lapply(names(rf_cv_binary_list[[x]]), function(a) {
    xauc <- rocs_list[[x]][[a]]$auc |> as.numeric()
    x1 <- confusion_matrices[[x]][[a]]$overall[c("Accuracy", "Kappa")]
    x2 <- confusion_matrices[[x]][[a]]$byClass
    c(AUC = xauc, x1, x2)
  })
  names(metr) <- names(rf_cv_binary_list[[x]])
  metr
})
names(metrics_binary) <- names(to_do_list)

# prepare a dataframe
accuracy_ranger_wide <- lapply(seq_along(to_do_list), function(x) {
  cbind(
    to_do_list[[x]][
      to_do_list[[x]]$response_type == "binary",
      c("rf_short", "technical_predictors", "age_and_gender")
    ],
    sapply(
      X = to_do_list[[x]]$rf_formula[to_do_list[[x]]$response_type == "binary"],
      FUN = function(y) metrics_binary[[x]][[y]]
    ) |>
      t() |> as.data.frame()
  )
})
names(accuracy_ranger_wide) <- names(to_do_list)

# reshape into long format
accuracy_ranger_long <- lapply(
  X = accuracy_ranger_wide, FUN = reshape,
  varying = c(
...
accuracy_ranger_long <- lapply(accuracy_ranger_long, function(x) {
  x$metric_type <- factor( # metric type as factor
    x$metric_type,
    levels = c(
      "AUC", "Accuracy", "Kappa", "Sensitivity", "Specificity",
      "Pos Pred Value", "Neg Pred Value", "Precision", "Recall",
  ...
  levels(x$metric_type) <- c(
    "Accuracy", "Sensitivity", "Specificity", "PPV", "NPV"
  )
  x
})

# barplot of accuracy metrics
for (a in seq_along(to_do_list)) {
  # prepare coordinates for barplot
  xx <- barplot(
    metric ~ rf_short + metric_type,
    data = accuracy_ranger_long[[a]],
    beside = TRUE, plot = FALSE
  )
  rownames(xx) <- levels(accuracy_ranger_long[[a]]$rf_short)
  colnames(xx) <- levels(accuracy_ranger_long[[a]]$metric_type)
  # create barplot
  png(file.path(outdirs[a], "barplot_metrics_binary.png"),
  ...
  for (x in seq_len(nrow(xx))) {
    for (y in seq_len(ncol(xx))) {
      yvalue <- fakeyvalue <- accuracy_ranger_long[[a]]$metric[
        as.numeric(accuracy_ranger_long[[a]]$rf) == x &
          as.numeric(accuracy_ranger_long[[a]]$metric_type) == y
      ]
  ...
  mtext(
    text = levels(accuracy_ranger_long[[a]]$metric_type),
...
rm(a, x, y, yvalue, xx, ylabs)


# Plot confusion matrices -------------------------------------------------
for (x in seq_along(to_do_list)) {
  for (i in seq_len(nrow(to_do_list[[x]]))) {
    xform <- to_do_list[[x]]$rf_formula[i]
    xx <- confusion_matrices[[x]][[xform]]$table |> as.data.frame()
    ...
    plot_confusion_matrix(xcm,
      ...
      filename = file.path(to_do_list[[x]]$folder[i], paste0(
        "CM_plot_", to_do_list[[x]]$rf_formula[i], ".png"
      )),
...
for (x in seq_along(to_do_list)) {
  tab2 <- lapply(to_do_list[[x]]$rf_formula, FUN = function(a) {
    c(
      n_train = nrow(df_train_list[[x]][[a]]),
      tab_train = table(df_train_list[[x]][[a]]$response),
      n_test = nrow(df_test_list[[x]][[a]]),
      tab_test = table(df_test_list[[x]][[a]]$response)
    )
  }) |>
    do.call(what = rbind) |>
    as.data.frame()
  tab2 <- cbind(RF = to_do_list[[x]]$rf_short, tab2)

  # save table as excel
  writexl::write_xlsx(
    tab2,
    file.path(outdirs[x], "sample_sizes_rawtable.xlsx"),
    format_headers = TRUE
  )
  ...
  gtsave(xtab, file.path(outdirs[x], "sample_sizes_table.png"))
}

# Save image --------------------------------------------------------------
save.image("nonsync/08b_plots_RF_metadaset.RData")
