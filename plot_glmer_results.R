...
plot_glmer_fitted_multiple_continuous <- function(
  ...
  # over the range of values for that PC
  # with all other model predictors being centered
  fits <- lapply(covariates_to_plot, function(x) {
    fit.fun(
      m = model, data = data, pred.data = NULL,
      boots = bootstrap_object$all.boots, link = link,
      use = x, # get fitted values over the range of the covariate to be plotted
      center = setdiff(fe_predictors, x), # center all other predictors
      resol = fitted_resolution, level = 0.95,
      offset2add = NULL, keep.boots = FALSE
    )
  })
  names(fits) <- covariates_to_plot


  ### prepare stats ###
  stats <- lapply(covariates_to_plot, function(x) {
    r <- results_table[x, , drop = FALSE]
    beta <- as.numeric(r[, "Estimate"])
    lcl <- as.numeric(r[, "lower_CI"])
    ucl <- as.numeric(r[, "upper_CI"])
    OR <- as.numeric(r[, "OR"])
  ...
  names(stats) <- covariates_to_plot
  ...
  # graphical parameters
  plotmat <- matrix(seq_len(length(covariates_to_plot) * 2), nrow = 2, byrow = FALSE)
  layout(plotmat, heights = c(3, 1))


  # loop over each covariate to plot
  for (covariate_to_plot in covariates_to_plot) {
    par( # graphical parameters
      las = 1, mar = c(2.5, 2.5, 0.5, 2.5),
      mgp = c(1.5, 0.8, 0), tcl = -0.3, xpd = FALSE
    )
    ...
    axis(4, at = pretty(0:1))
    mtext(4,
      text = paste0("p(", response_levels[2], ")"), las = 0,
      line = par("mgp")[1], cex = 0.7
    )

    # CIs of the fitted values
    polygon(
      x = c(fits[[covariate_to_plot]][, covariate_to_plot], rev(fits[[covariate_to_plot]][, covariate_to_plot])),
      y = c(fits[[covariate_to_plot]]$lwr, rev(fits[[covariate_to_plot]]$upr)),
      border = NA, col = adjustcolor("#023E8A", 0.2)
    )
    lines(x = fits[[covariate_to_plot]][, covariate_to_plot], y = fits[[covariate_to_plot]]$fit, lwd = 2, col = "#023E8A")

    # add statistics
    par(mar = c(0.5, 2.5, 0.5, 2.5), xpd = TRUE)
    plot(NULL, xlim = c(-1, 1), ylim = c(-1, 1), axes = FALSE, ann = FALSE)
    legend(
      x = 0, y = 0,
      legend = stats[[covariate_to_plot]], text.col = "black", cex = 0.9, bty = "n",
      xjust = 0.5, yjust = 0.5,
      title = paste("Model results for", covariate_to_plot)
    )
  }
