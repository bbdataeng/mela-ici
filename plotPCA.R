...
plot_pca_factor <- function(
  ...
  if (!file_format %in% supported_formats) {
    stop("File format not supported. supported formats are: ",
  ...
  for (xvar in vars_to_use) {
    ...
    if (length(colors_vars_list[[xvar]]) != nlevels(metadata[,xvar])) {
      stop("The length of 'colors_vars_list$", xvar,
    ...
    if(!all(names(colors_vars_list[[xvar]]) %in% levels(metadata[,xvar]))) {
      stop("The names of 'colors_vars_list$", xvar,
  ...
  for (xvar in vars_to_use) {
    ...
    if (file_format == "pdf") {
      pdf(file = paste(filename, file_format, sep = "."),
          width = width_in, height = height_in)
    } else if (file_format == "png") {
      png(filename = paste(filename, file_format, sep = "."),
          width = res_ppi*width_in, height = res_ppi*height_in, res = res_ppi)
    } else if (file_format == "jpeg" | file_format == "jpg") {
      jpeg(filename = paste(filename, file_format, sep = "."),
           width = res_ppi*width_in, height = res_ppi*height_in, res = res_ppi)
    } else if (file_format == "tiff") {
      tiff(filename = paste(filename, file_format, sep = "."),
           width = res_ppi*width_in, height = res_ppi*height_in, res = res_ppi)
    } else if (file_format == "bmp") {
      bmp(filename = paste(filename, file_format, sep = "."),
          width = res_ppi*width_in, height = res_ppi*height_in, res = res_ppi)
    } else {stop("Unknown error when opening the graphic device")}
    ...
    # create plot
    plot(x = xpcs[,PCs_to_plot[1]],
         y = xpcs[,PCs_to_plot[2]],
         pch = 21, cex = pt.cex, bty = "o", axes = TRUE,
         asp = ifelse(equal_axis_scale, 1, NA),
         ylab = paste0(PCs_to_plot[2], ": ", percent_var[PCs_to_plot[2]],
           "% of total variation"),
         xlab = paste0(PCs_to_plot[1], ": ", percent_var[PCs_to_plot[1]],
                       "% of total variation"),
